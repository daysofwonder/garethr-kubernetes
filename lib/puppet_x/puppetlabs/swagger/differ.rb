...
# [["-", "progressDeadlineSeconds", 600], ["-", "replicas", 1], ["-", "revisionHistoryLimit", 10], ["-", "strategy", {:type=>"RollingUpdate", :rollingUpdate=>{:maxUnavailable=>"25%", :maxSurge=>"25%"}}], ["-", "template.metadata.creationTimestamp", nil], ["+", "template.metadata.annotations", {:"ad.datadoghq.com/app.check_names"=>"[\"openmetrics\"]", :"ad.datadoghq.com/app.init_configs"=>"[{}]", :"ad.datadoghq.com/app.instances"=>{:prometheus_url=>"http://%%host%%:8184/metricz", :namespace=>"thoth", :metrics=>["go_*", "http_*"]}}], ["-", "template.spec.dnsPolicy", "ClusterFirst"], ["-", "template.spec.restartPolicy", "Always"], ["-", "template.spec.schedulerName", "default-scheduler"], ["-", "template.spec.securityContext", {}], ["-", "template.spec.terminationGracePeriodSeconds", 30], ["-", "template.spec.containers[0].imagePullPolicy", "Always"], ["-", "template.spec.containers[0].resources", {}], ["-", "template.spec.containers[0].terminationMessagePath", "/dev/termination-log"], ["-", "template.spec.containers[0].terminationMessagePolicy", "File"], ["~", "template.spec.containers[0].env[0].name", "TEST", "TEST12"], ["+", "template.spec.containers[0].env[1]", {:name=>"TEST4", :value=>"VALUE2"}], ["~", "template.spec.containers[0].name", "hello-world2", "hello-world"]]
