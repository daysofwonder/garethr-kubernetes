...
Puppet::Type.type(:kubernetes_persistent_volume_claim_condition).provide(:swagger, :parent => PuppetX::Puppetlabs::Kubernetes::Provider) do
  ...
  def create
    Puppet.info("Creating kubernetes_persistent_volume_claim_condition #{name}")
    create_instance_of('persistent_volume_claim_condition', name, build_params)
  end

  def flush
   unless @property_hash.empty?
     unless resource[:ensure] == :absent
        flush_instance_of('persistent_volume_claim_condition', name, @property_hash[:object], build_params)
      end
    end
  end

  def destroy
    Puppet.info("Deleting kubernetes_persistent_volume_claim_condition #{name}")
    destroy_instance_of('persistent_volume_claim_condition', name)
    @property_hash[:ensure] = :absent
  end

  private
  def self.list_instances
    list_instances_of('persistent_volume_claim_condition')
  end
  ...
  def build_params
    params = {
        ...
        lastProbeTime: resource[:last_probe_time],
