...
Puppet::Type.type(:kubernetes_stateful_set_condition).provide(:swagger, :parent => PuppetX::Puppetlabs::Kubernetes::Provider) do
  ...
  def create
    Puppet.info("Creating kubernetes_stateful_set_condition #{name}")
    create_instance_of('stateful_set_condition', name, build_params)
  end

  def flush
   unless @property_hash.empty?
     unless resource[:ensure] == :absent
        flush_instance_of('stateful_set_condition', name, @property_hash[:object], build_params)
      end
    end
  end

  def destroy
    Puppet.info("Deleting kubernetes_stateful_set_condition #{name}")
    destroy_instance_of('stateful_set_condition', name)
    @property_hash[:ensure] = :absent
  end

  private
  def self.list_instances
    list_instances_of('stateful_set_condition')
  end
