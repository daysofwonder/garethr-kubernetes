...
Puppet::Type.type(:kubernetes_scale_io_volume_source).provide(:swagger, :parent => PuppetX::Puppetlabs::Kubernetes::Provider) do
  ...
  def create
    Puppet.info("Creating kubernetes_scale_io_volume_source #{name}")
    create_instance_of('scale_io_volume_source', name, build_params)
  end

  def flush
   unless @property_hash.empty?
     unless resource[:ensure] == :absent
        flush_instance_of('scale_io_volume_source', name, @property_hash[:object], build_params)
      end
    end
  end

  def destroy
    Puppet.info("Deleting kubernetes_scale_io_volume_source #{name}")
    destroy_instance_of('scale_io_volume_source', name)
    @property_hash[:ensure] = :absent
  end

  private
  def self.list_instances
    list_instances_of('scale_io_volume_source')
  end
